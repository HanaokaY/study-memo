...
# だから、serverと付くものは「TCPS」「UNIX」だけということを覚えておけばいい。それ以外は存在するならsocketが名前につく



# 【StringIO】
# StringIOに関しては問題を解くだけでほとんど何も学習してません。
# ので、何も述べることができませんが、下記のような問題パターンは出題されました。
# というかStringIOの問題は下記のような問題パターンしか見たことありません。
# ので、StringIOについては下記のような問題パターンに答えれるようにしておくぐらいにして、
# あとは他に注力すべきだと思いました(試験を受けた所感)。なので問題だけ載せておきます。
...
# Thread スレッド
    ...
    # sleep => 停止中
    # aborting => 終了処理
    # false => 正常終了したスレッドの戻り値
    # nil => 例外などで終了した場合
    # dead => 終了状態(Thread#killで終了した場合)
...
#     p "foo"
#     p f.resume #=> Fiber.yieldがない場合は、nilが出力される。
#     p "bar"
