...
# begin
#     raise Err2
# rescue => e #=> ここですべてのエラーはキャッチされてしまう。Standarderrorの派生だから
#     p 'StandardError'    
# rescue Err2 => ex
#     p ex.class
# end
...
class C
    ...
    VAR = 100 #=> 別に上書き自体は問題なくできる

end

c = C.new
# C.VAR=3 #=> method内での定数の変更は文法エラー
p c.VAR



# do-endの時は()を省略することができる。{}では()は省略できない。
1.upto 5 do |x| p x end #=> これはオッケー
# 1.upto 5 { |x| p x } #=> これは文法エラー

def tag(name,&block)
    puts "<#{name}>#{block[]}<#{name}>"
end
tag(:p){'hello'}
# 引数で&blockとして受け取っている場合、block.callまたはblock[]で呼び出せる。
# yieldでは、引数に&blockがなくても呼び出せるが、yield.callはできない。
